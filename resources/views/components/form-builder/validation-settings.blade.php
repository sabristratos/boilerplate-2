    ...
    @else
        ...
        <div class="space-y-3">
            ...
            @if(in_array('required', $selectedRules))
                                        <flux:input 
                            wire:model.blur="draftElements.{{ $selectedElementIndex }}.validation.messages.required" 
            ...
            <div class="space-y-3">
                ...
                <div class="flex gap-2">
                    @foreach($groupedRules['Length'] as $ruleKey => $rule)
                        @if($rule['has_value'])
                            <flux:input 
                                wire:model.blur="draftElements.{{ $selectedElementIndex }}.validation.values.{{ $ruleKey }}" 
                                label="{{ $rule['label'] }}" 
                ...
                @foreach($groupedRules['Length'] as $ruleKey => $rule)
                    @if($rule['has_value'] && !empty($ruleValues[$ruleKey]))
                        <flux:input 
                            wire:model.blur="draftElements.{{ $selectedElementIndex }}.validation.messages.{{ $ruleKey }}" 
                            label="Custom Message (optional)" 
                            placeholder="Custom error message..."
                            size="sm"
                            help="Leave empty to use default message"
        ...
        @if(isset($groupedRules['Format']))
            <div class="space-y-3">
                ...
                @foreach($groupedRules['Format'] as $ruleKey => $rule)
                    @if($ruleKey === 'regex')
                        <flux:input 
                            wire:model.blur="draftElements.{{ $selectedElementIndex }}.validation.values.regex" 
                        ...
                        @if(!empty($ruleValues['regex']))
                            <flux:input 
                                wire:model.blur="draftElements.{{ $selectedElementIndex }}.validation.messages.regex" 
                                label="Custom Message (optional)" 
                                placeholder="Custom error message..."
                                size="sm"
                                help="Leave empty to use default message"
